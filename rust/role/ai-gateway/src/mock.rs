...
pub struct MockProvider {
    script: Vec<AiResponse>,
    next: Mutex<usize>,
}

impl MockProvider {
    /// Build a mock that answers turn `i` with `script[i]`.
...
impl Provider for MockProvider {
    async fn complete(&self, _req: &AiRequest) -> AiResponse {
        ...
        *next += 1;
        self.script
            .get(i)
            .cloned()
            .unwrap_or_else(|| AiResponse::stop("(mock: script exhausted)", 0))
    }
}
