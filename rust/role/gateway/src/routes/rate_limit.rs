...
//! before any handler logic runs; the three `/public/case/request*` routes are a token oracle for
//! the same reason and carry the same ceiling; `GET /public/branding` is neither, but it is still an
...
// ---------------------------------------------------------------------------------------------
// The `/public/case/request*` per-IP limiter (case-plane scope wave 2). A SEPARATE bucket from the
// invite limiter, on purpose: a contractor loading their page, uploading a photo and replying spends
// several requests in a minute, and sharing one counter would let ordinary use of either route lock
// the other out. Same ceiling and window as the invite route — the token route is a token oracle for
// exactly the reason the invite route is.

/// The one process-wide limiter for the public case-request routes.
...
pub async fn case_request_rate_limit(req: Request, next: Next) -> Response {
    limit(case_request_limiter(), req, next).await
}
