...
use crate::routes::{
    ...
    upload_body_limit, upload_pack, upload_status, write_samples,
};
...
pub fn router(gw: Gateway) -> Router {
    ...
    let router = Router::new()
        ...
        .route("/insights/{id}", get(get_insight).delete(delete_insight))
        ...
        .route(
            "/insights/{id}/occurrences/{oseq}",
            axum::routing::delete(delete_occurrence),
        )
        ...
        // invite-admin-routes scope — the AUTHENTICATED half of the invite family: mint, list,
        // revoke, resend. The pre-auth redeem half (`POST /public/invite/accept`, `GET
        // /public/invite/verify`) is registered ABOVE, unauthenticated and IP-rate-limited; these
        // are its counterpart, so a browser holding a session bearer can work the mint side of the
        // same door. Caps are re-checked server-side inside the host verb — `mcp:invite.list:call`
        // for the roster, `mcp:invite.create:call` for mint/revoke/resend (two caps, not one). ws +
        // principal from the token; the path carries the `token_hash`, never a redeemable token.
        // No rate limiter here: these sit behind the bearer AND an admin cap, and throttling would
        // punish an admin bulk-onboarding a site.
        .route("/admin/invites", get(list_invites).post(create_invite))
        .route("/admin/invites/{token_hash}/revoke", post(revoke_invite))
        .route("/admin/invites/{token_hash}/resend", post(resend_invite))
        ...
        .route("/dashboards/{id}/shares", get(list_shares_dashboard))
        .route("/dashboards/{id}/unshare", post(unshare_dashboard))
        ...
        .route("/panels", get(list_panels).post(save_panel))
        ...
        // entity version history (versions scope, #112) — list / inspect / restore the newest N
        // snapshots of a dashboard, flow, or rule. `kind` is an OPAQUE path segment: the host's kind
        // plan table decides what exists, so a future kind needs no route change. `/versions/config`
        // is two segments and the list is three, so they cannot shadow each other.
        .route(
            "/versions/config",
            get(get_versions_config).put(put_versions_config),
        )
        .route("/versions/{kind}/{id}", get(get_versions))
        .route("/versions/{kind}/{id}/{version_id}", get(get_version))
        .route(
            "/versions/{kind}/{id}/{version_id}/restore",
            post(post_version_restore),
        )
