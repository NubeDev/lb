...
use crate::wrapper::{AgentWrapper, Decoded, McpConfig};
...
impl AgentWrapper for CodexWrapper {
    ...
    /// Codex reads its MCP-server config from a `config.toml` under `CODEX_HOME` (defaults to
    /// `~/.codex`; the role crate sets `CODEX_HOME=<scratch>/codex-home`). The file is a TOML table
    /// with one `[mcp_servers.<id>]` stanza per server; each stanza names a `command`, optional
    /// `args`, and an `env` table of env vars to pass to the shim child. The bridge ENV vars
    /// (`LB_MCP_*`) are set on the codex child by the role crate and INHERITED by the shim — they
    /// are NOT duplicated here (codex's `env` table is for vars the shim should NOT inherit from
    /// the parent, which is the opposite of what we want).
    ///
    /// We return the `config.toml` contents; the role crate writes it at
    /// `<scratch>/codex-home/config.toml` and the codex argv (`-C` is `--cd`, the cwd; the config
    /// dir is `CODEX_HOME`). A `stdio` transport is the only one the shim speaks.
    fn mcp_config(&self, shim_bin: &str, scratch: &Path) -> Option<McpConfig> {
        let home = scratch.join("codex-home");
        let _ = std::fs::create_dir_all(&home);
        let rel = home.join("config.toml");
        let contents = format!(
            "# Generated by lb-role-external-agent. Per-run; deleted with the scratch dir.\n\
             # The shim is codex's ONLY configured MCP server — every tool call reaches the\n\
             # gateway, which re-checks caps exactly as it does for the UI.\n\
             [mcp_servers.lb]\n\
             command = \"{shim_bin}\"\n\
             args = []\n\
             # `env` is empty by design: the LB_MCP_* bridge vars are INHERITED from the codex\n\
             # child (the role crate sets them on codex's env), not duplicated here. Setting them\n\
             # here would expose the token in this file's env table unnecessarily.\n\
             env = {{}}\n"
        );
        Some(McpConfig {
            // The path RELATIVE to the scratch dir, so the role crate writes it at
            // `<scratch>/<file_name>`. `codex-home/config.toml`.
            file_name: rel
