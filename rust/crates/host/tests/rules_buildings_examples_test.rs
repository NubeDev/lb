...
/// The absolute path to the seeded demo dataset under `.lazybones/data/demo/`, or `None` when it has
/// not been generated on this machine.
///
/// This previously declared the file "a committed fixture" and refused to skip. It is not committed
/// and cannot be: `.lazybones/` is git-ignored (`.gitignore`), and the dataset is GENERATED by
/// `docker/postgres/seed-demo-sqlite.sh`, which needs a RUNNING node with the federation sidecar plus
/// `jq`/`curl`/`python3`. No CI job does that, so the test could never pass there — it was permanently
/// red, which is the state that makes a suite unreadable (a real new failure looks identical to the
/// standing one).
///
/// So: skip, loudly, exactly as `federation_test` does when Docker is unavailable. The skip prints the
/// command that produces the fixture, so a developer who wants the coverage can get it in one step,
/// and CI stops reporting a missing generated artifact as a product failure.
fn buildings_db() -> Option<String> {
...
async fn buildings_examples_run_end_to_end() {
    ...
    let Some(db) = buildings_db() else {
        eprintln!(
            "SKIP buildings_examples_run_end_to_end: .lazybones/data/demo/buildings.db is absent. \
             It is generated, not committed (.lazybones/ is git-ignored). Produce it with:\n  \
             make seed-demo-sqlite      (needs a running `make dev` node + jq/curl/python3)"
        );
        return;
    };
    let ws = "nube";
