...
async fn ws_b_cannot_read_ws_a_doc() {
    ...
    // Test owns a doc in workspace A.
    let test_a = principal(
        "user:test",
        "ws-iso-a",
    ...
    let test_b = principal("user:test", "ws-iso-b", &["store:doc/*:read"]);
    let err = get_doc(&store, &test_b, "ws-iso-b", "scope-x")
        .await
        .unwrap_err();
    ...
    let cross = principal("user:test", "ws-iso-b", &["store:doc/*:read"]);
    assert!(matches!(
...
async fn ws_b_list_never_returns_ws_a_docs() {
    ...
    let test_b = principal("user:test", "ws-iso-list-b", &["store:doc/*:read"]);
    assert!(list_docs(&store, &test_b, "ws-iso-list-b")
        .await
        .unwrap()
        .is_empty());
}
...
async fn ws_b_cannot_load_ws_a_skill() {
    ...
    put_skill(&store, &test_a, "ws-iso-skill-a", "s", "1.0.0", "d", "B", 1)
        .await
        .unwrap();
    grant_skill(&store, &test_a, "ws-iso-skill-a", "s")
        .await
        .unwrap();
    ...
    let test_b = principal("user:test", "ws-iso-skill-b", &["store:skill/*:read"]);
    assert!(matches!(
        load_skill(&store, &test_b, "ws-iso-skill-b", "s", None)
            .await
            .unwrap_err(),
    ...
    assert!(matches!(
        load_skill(&store, &test_b, "ws-iso-skill-a", "s", None)
            .await
            .unwrap_err(),
