...
const ALL: &[&str] = &["mcp:ingest.write:call", "mcp:series.read:call", "mcp:series.latest:call"];
...
async fn ws_b_cannot_read_ws_a_series() {
    ...
    call_ingest_tool(&store, &a, "ws-a", "ingest.write", &json!({ "samples": [sample("secret", 1, json!(42))] }))
        .await
        .unwrap();
    ...
    let read = call_ingest_tool(&store, &b, "ws-b", "series.read", &json!({ "series": "secret" }))
        .await
        .unwrap();
    assert!(read["samples"].as_array().unwrap().is_empty(), "ws-B must not see ws-A samples");

    // And a ws-B token asking for ws-A's namespace is refused at gate 1 (workspace), opaque Denied.
    let cross = call_ingest_tool(&store, &b, "ws-a", "series.read", &json!({ "series": "secret" }))
        .await
        .unwrap_err();
...
async fn ws_b_cannot_write_ws_a_series() {
    ...
    let err = call_ingest_tool(&store, &b, "ws-a", "ingest.write", &json!({ "samples": [sample("m", 1, json!(1))] }))
        .await
        .unwrap_err();
    ...
    drain_workspace(&store, "ws-a").await.unwrap();
    let read = call_ingest_tool(&store, &admin_a, "ws-a", "series.read", &json!({ "series": "m" }))
        .await
        .unwrap();
...
async fn two_producers_same_seq_both_survive_via_host() {
    ...
    call_ingest_tool(&store, &pa, "acme", "ingest.write", &json!({ "samples": [sample("shared", 5, json!("a"))] }))
        .await
        .unwrap();
    call_ingest_tool(&store, &pb, "acme", "ingest.write", &json!({ "samples": [sample("shared", 5, json!("b"))] }))
        .await
        .unwrap();
    drain_workspace(&store, "acme").await.unwrap();

    let read = call_ingest_tool(&store, &pa, "acme", "series.read", &json!({ "series": "shared" }))
        .await
        .unwrap();
