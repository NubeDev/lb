...
use lb_authz::{resolve_caps_sourced, resolve_subject_caps_sourced, SourcedCap, Subject};
...
pub async fn authz_resolve(
    ...
    match subject {
        Subject::User(user) => Ok(resolve_caps_sourced(store, ws, user).await?),
        _ => Ok(resolve_subject_caps_sourced(store, ws, subject).await?),
    }
}
