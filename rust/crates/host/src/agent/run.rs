...
use lb_jobs::{append_event, cancel, complete, create, load, Job, JobStatus, TranscriptEvent};
use lb_mcp::call;
...
use lb_run_events::{project_one, RunEvent, RunOutcome};
...
/// Re-read the durable job to see whether it was cancelled mid-run. Cheap at S5 scale (one record);
/// a tighter cancellation signal (a bus subject) is a follow-up, but the durable check is the
/// authority.
async fn is_cancelled(node: &Node, ws: &str, job_id: &str) -> Result<bool, AgentError> {
...
/// Append a transcript event durably AND emit its live [`RunEvent`] projection onto the run's bus
/// subject (agent-run scope Part 3). The order is deliberate: the durable append is the **record**
/// (it must land, hence `?`), then the bus publish is **motion** (best-effort — a watcher that misses
/// it catches up from the transcript snapshot, §3.3). `turn` is the loop's current turn number, so a
/// `StepStart` carries the right index; `project_one` is the SAME projection the snapshot uses, so a
/// live watcher and a reconnecting one can never diverge (Part 1, review point 5).
async fn emit(
    node: &Node,
    ws: &str,
    job_id: &str,
...
/// How many model turns the transcript already records — one per [`AssistantTurn`]. Used to resume
/// the turn counter (and so the ceiling + idempotency key) at the right place.
fn count_turns(events: &[&TranscriptEvent]) -> u32 {
    events
        .iter()
