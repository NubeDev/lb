...
//! scope). A native Tier-2 sidecar (spawned by `lb-supervisor` with its scoped identity in env) uses
//! this to CALL host MCP tools — `ingest.write`, `outbox.enqueue`, `series.find`, any tool its grant
//! includes — the same way an edge user does: an authenticated `POST /mcp/call` to the gateway. It is
//! the out-of-process peer of the wasm guest's in-process `host.call-tool` bridge; both are transports
//! for the ONE MCP contract (rule 7), each denied identically by the host's capability + workspace gate.
//!
//! Nothing here is `ros`/`fleet`-specific: it is the generic mechanism `ros`'s poller (`ingest.write`)
//! and `point.write` (`outbox.enqueue`), and later `mqtt`/`control-engine`, all reach the host through.
//! The child authenticates with the node-signed `LB_EXT_TOKEN` (verifiable by the gateway since the
//! minter and verifier now share the node key); it can reach only its `requested ∩ admin_approved`
//! grant — a capability denial surfaces as [`CallError::Denied`], never a panic.
