...
pub enum ToolError {
    ...
    #[error("denied")]
    Denied,
    /// Authorization failed for a caller who had **already passed the target's read gates** — so the
    /// refusal may name its cause without leaking anything the caller could not already read.
    ///
    /// The narrow sibling of [`ToolError::Denied`], on the same footing as [`ToolError::Ambiguous`]:
    /// structured detail is safe precisely because it is unreachable from an unauthorized caller. A
    /// verb that cannot prove the caller's read access MUST return the opaque `Denied` instead.
    ///
    /// `code` is a stable machine token the client branches on (e.g. `"managed"` — the write was
    /// refused because the asset is generated by something else); `subject` is the value it renders
    /// (e.g. the managing extension's bare id). Generic on purpose: the MCP layer transports the
    /// pair and never interprets either half, so no domain concept leaks down here.
    #[error("denied: {code}={subject}")]
    DeniedBecause { code: String, subject: String },
