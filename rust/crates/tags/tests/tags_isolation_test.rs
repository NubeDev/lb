...
async fn identical_tag_does_not_leak_across_workspaces() {
    ...
    add(&store, "ws-a", "series:a", &region_eu, &Provenance::new(1, "user:a", Source::Producer), 0)
        .await
        .unwrap();
    add(&store, "ws-b", "series:b", &region_eu, &Provenance::new(1, "user:b", Source::Producer), 0)
        .await
        .unwrap();

    // ws-B find for region=eu must return ONLY ws-B's entity — never ws-A's, despite the same node id.
    let b_hits = find(&store, "ws-b", &[Facet::exact("region", "eu".into())]).await.unwrap();
    assert_eq!(b_hits, vec!["series:b".to_string()], "ws-B sees only its own edge");

    // And vice versa.
    let a_hits = find(&store, "ws-a", &[Facet::exact("region", "eu".into())]).await.unwrap();
    assert_eq!(a_hits, vec!["series:a".to_string()], "ws-A sees only its own edge");
