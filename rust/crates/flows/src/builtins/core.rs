        ...
        NodeDescriptor::new("flipflop", NodeKind::Trigger, "")
            ...
            .with_config(
                ...
                json!({
                    ...
                    "additionalProperties": false,
                    "properties": {
                        // `minimum: 1` — the per-node interval timer (interval-source-clock scope,
                        // Phase 2 / Option A) has landed, so the 5s reactor-sweep floor is GONE: each
                        // flip-flop owns a real timer and `period_secs: 1` means one second. The floor
                        // is now the durable cursor's unit (whole seconds), not a deployment constant.
                        // Sub-second (fractional) needs a millisecond cursor field AND a run-retention
                        // policy for high-frequency sources — a 100ms oscillator is 10 real runs/sec.
