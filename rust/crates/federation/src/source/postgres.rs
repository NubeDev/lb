...
impl Source for PostgresSource {
    ...
    async fn query_direct(&self, sql: &str) -> Result<Vec<RecordBatch>, SourceError> {
        // A `numeric` column decodes into an Arrow decimal whose scale is fixed by the FIRST row and
        // every later row is ROUNDED to it, so a whole-number first row truncates the whole column
        // (`20, 17.685, 15.334` → `20.0, 18.0, 15.0`). Only the server sees every row before it
        // sends any, so it is the only place that can choose a scale: ask it for `float8`, which is
        // what these cells become on the way out to JSON regardless. See `pg_numeric`.
        let widened = super::pg_numeric::widen_numeric_columns(&self.pool, sql).await;
    ...
    async fn query_direct_json(
        ...
        Ok(super::pg_cells::batches_to_column_rows(&batches))
    }
