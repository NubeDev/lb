...
async fn write_read_round_trips_on_disk() {
    ...
    assert_eq!(read(&store, "acme", "thing", "missing").await.unwrap(), None);
    cleanup(&path);
}
...
async fn ws_b_cannot_read_ws_a_record_on_disk() {
    ...
    cleanup(&path);
    let store = Store::open(&path).await.unwrap();
    write(&store, "ws-a", "secret", "x", &serde_json::json!({"who": "a"}))
        .await
        .unwrap();
    ...
    write(&store, "ws-b", "secret", "x", &serde_json::json!({"who": "b"}))
        .await
        .unwrap();
    assert_eq!(
...
async fn list_filters_by_field_on_disk() {
    ...
    cleanup(&path);
    let store = Store::open(&path).await.unwrap();
    write(&store, "acme", "inbox", "a", &serde_json::json!({"channel": "c1", "ts": 1}))
        .await
        .unwrap();
    write(&store, "acme", "inbox", "b", &serde_json::json!({"channel": "c2", "ts": 2}))
        .await
        .unwrap();
    let c1 = list(&store, "acme", "inbox", "channel", "c1").await.unwrap();
    assert_eq!(c1.len(), 1);
...
async fn write_tx_is_atomic_on_disk() {
    ...
    write_tx(
        &store,
        "acme",
        &Upsert { table: "domain", id: "d1", value: &cv },
        &Upsert { table: "outbox", id: "e1", value: &ev },
    )
    .await
    .unwrap();
    assert_eq!(read(&store, "acme", "domain", "d1").await.unwrap(), Some(cv));
    assert_eq!(read(&store, "acme", "outbox", "e1").await.unwrap(), Some(ev));
    cleanup(&path);
}
