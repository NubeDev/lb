...
use crate::chrome::{cover_page, index_block, running_footer, running_header};
use crate::convert::{markdown_to_typst, typst_string};
use crate::error::RenderError;
use crate::model::Assembled;
use crate::place::placed_page;
...
pub fn render_pdf(assembled: &Assembled) -> Result<Vec<u8>, RenderError> {
    ...
    // Placements ⇒ absolute layout (a report's A4 grid); otherwise flow the markdown as always.
    let body = assembled
        .pages
        .iter()
        .enumerate()
