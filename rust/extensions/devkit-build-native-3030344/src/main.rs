...
async fn main() {
    ...
    let ext_id = std::env::var("LB_EXT_ID").unwrap_or_else(|_| "devkit-build-native-3030344".into());
    let mut input = stdin();
    ...
    loop {
        ...
        let reply = match req.method {
            Method::Init => Reply::ok(req.id, format!(r#"{{"ready":true,"ext":"devkit-build-native-3030344"}}"#)),
...
fn handle_call(req: &Request, ws: &str, ext_id: &str) -> Reply {
    ...
    match params.tool.as_str() {
        "ping" => Reply::ok(
            req.id,
            format!(r#"{{"ok":true,"ext":"devkit-build-native-3030344","runtime_ext":"{ext_id}","ws":"{ws}","tier":"native"}}"#),
        ),
