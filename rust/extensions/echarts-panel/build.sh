#!/usr/bin/env sh
# Build the `echarts-panel` Tier-1 WASM extension — a wasm32-wasip2 component (excluded from the host
# workspace, built in its own target/) plus its federated UI bundle. Emits:
#   target/wasm32-wasip2/release/echarts_panel_ext.wasm  (the component the host loads)
...
echo "built: $(pwd)/target/wasm32-wasip2/release/echarts_panel_ext.wasm"
