...
```
Dashboard cell (view: "template")
  └─ ScriptedView                  resolves code: inline options.code  OR  template.get(templateId)
       └─ WidgetIframe             PARENT side of the bridge; sandboxed <iframe>
            │  frame.srcdoc = buildIframeSrcdoc({ engine, code, tools })
            │
            │   ── postMessage {type:"bridge-call", id, tool, args} ──▶
            ▼
       iframe (opaque origin, sandbox="allow-scripts")
            ├─ root.innerHTML = code            (template engine: eval-free)
            └─ [data-call] buttons → bridge.call(tool, args)
            │
...
The frame **cannot reach the network directly** (CSP `connect-src 'none'`). Every read
or write goes parent → host, and the **session token never enters the frame**.
...
## The `template` engine (eval-free)

Unlike `plot`/`d3` (which run author code via `new Function(...)`), the `template` engine
does **no eval** ([iframeRuntime.ts:110-126](../../../../ui/src/features/dashboard/builder/iframeRuntime.ts#L110-L126)):

```js
if (cfg.engine === "template") {
  window.__bridge = bridge;
  root.innerHTML = cfg.code;                        // author HTML, verbatim
...
So the template contract is:

1. **Markup** is set as `innerHTML` (the CSP + opaque origin bound what it can do).
2. **Write buttons** are declared with attributes:
   ```html
...
([TemplateSourceField.tsx:20-23](../../../../ui/src/features/dashboard/builder/editors/TemplateSourceField.tsx#L20-L23)):
...
> **Gotcha — `{{path}}` is convention, not (yet) a substitution pass.** The comments and
> the default snippet describe `{{rows.length}}`-style interpolation over a `data` object,
> but the shipped `template` engine sets `root.innerHTML = cfg.code` **verbatim** and only
> wires `[data-call]` buttons. It does **not** currently run a `{{…}}` interpolation over
> author-fetched rows. Treat `{{…}}` as intended convention; if you rely on live
> interpolation, it needs to be implemented in `iframeRuntime.ts`.
...
Per CLAUDE §9 (no mocks), the coverage runs against a **real in-process gateway**
