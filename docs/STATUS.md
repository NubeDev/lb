...
- **Tests:** `role/gateway/tests/invite_admin_routes_test.rs` (the mandatory categories) +
  ...
  (`beta` sees nothing and gets a `404` — not a `403` — revoking `nube`'s hash), the mint →
...
`email_transport_test` 6 (cap-deny writing **no outbox row**, ws-isolation via sealed-in-globex vs nube,
...
host verb) + Range/304 serve + multi-chunk resume test, push **hardcoded `"nube"` workspace in
...
workspace-isolation (an `nube`-only source is unresolvable from ws `other`). `cargo fmt` clean.
...
(`user:test` → workspace-admin of `nube`, the core-skill/agent/persona catalogs, the four background
...
of `boot_full` on `127.0.0.1:0` + reqwest: login returns a real signed token for `user:test`/`nube`,
...
App.tsx; prefilled test/nube signs straight through to Channels. The preview node is in-memory, so a
...
· workspace isolation · **member wall (bob never sees member:test)** · idempotent upsert · bounds +
...
ids unchanged; needs node rebuild to surface). **Live-verified on `:8080`/`nube`/`chain4`:** converting
...
|---|---|---|---|---|---|---|
| **A curated menu ROW is pinnable — `nav:<navid>/<rowid>`** — a board bound to variables ("Chullora › Energy") and a folder that opens its own board had no pin ref, so a site menu drew no pin anywhere. | nav | post-S10 | **tested** (2026-09-17, branch `feat/nav-row-pins`, unreleased — needs the next `node-v*` tag) | [nav-row-pins](scope/nav/nav-row-pins-scope.md) | [nav-row-pins](sessions/nav/nav-row-pins-session.md) | `NavItem.id` (kept / minted / duplicate re-minted / malformed `BadInput` in `nav.save`, `row_ids.rs`); `ResolvedItem` gains `id` (echoed by `resolve_item`), and on a row pin `nav_id` + `trail`; a fifth pin grammar resolved in `resolve_row_pin.rs` through the pick tiers' readability gate + the ordinary `resolve_item` pipeline (a pinned folder keeps everything inside it; hide beats pin via `strip_hidden`; every miss strips silently). No new caps/verbs; no migration — an old nav's rows gain ids on their next save. Tests: `nav_row_pins_test` 5 new; `nav_test` 50 (one round-trip assertion now clears minted ids), `reusable_pages` 8, `nav_home` 8, `nav_footer` 3, `nav_context_builtins` 6, `nav_ext_boards_gate` 4, lib `nav` 35, gateway `nav_default_route` 3 + `nav_reach` 2 — all green. |
| **`filterByValue` gets an array-membership matcher** — a multi-value dashboard variable can now filter rows directly. | viz | post-S10 | **shipped** (2026-08-13, unreleased — needs the next `node-v*` tag) | [transformations](scope/frontend/dashboard/viz/transformations-scope.md) | [filter-by-value-one-of](sessions/frontend/filter-by-value-one-of-session.md) | `rust/crates/viz/src/transforms/filter_by_value.rs` gains **`oneOf`/`notOneOf`**: `options.value` is an ARRAY and the matcher tests membership (`meter` in `["MSB-2 Main","MSB-01"]`). These are **lb's own ids — Grafana has no equivalent**, the first deliberate departure from this transformer's verbatim-Grafana matcher set, so they are the one part of `filterByValue` a Phase-4 Grafana export cannot round-trip (flagged in the module header). **Why lb owns it:** filtering executes entirely server-side (the client posts `panel.transformations` to `viz.query` and re-implements no matcher), so the downstream editor affordance is useless without this. `equal`'s body was extracted to `equal_to(value, target)` and `oneOf` tests each member with it — membership is *equality N times*, and the shared helper is what stops a second equality semantic from drifting into existence. **Three judgement calls:** a **scalar** operand is a one-element list, because the client's URL round-trip collapses a single-element `?var-<name>=` selection back to a plain string — rejecting it would make the filter work at two selected rows and silently empty the panel at exactly one; an **empty** list matches NOTHING, so clearing a selection narrows rather than silently widening; `notOneOf` with no operand keeps every row, mirroring the shipped `notEqual` rather than diverging. **Known gap (named):** a variable's values are strings, so `oneOf` over a NUMERIC column compares `5` to `"5"` and misses — the same gap `equal` has carried since Phase 3. The fix is one change to `equal_to` covering all four matchers, deliberately not taken here because it would also make a text `"007"` match a target of `7`; `oneOf` forks no coercion of its own meanwhile. String columns (the actual ask) are unaffected. **Tests:** `cargo test -p lb-viz` 86 (79 + **7 new**, incl. the scalar round-trip, empty-list, and unresolved-`$var`-literal cases), `cargo fmt` clean. No caps/workspace surface touched — `lb-viz` is pure, and `viz.query`'s gate + wall are unchanged. **Downstream:** rubix-ai (pin `node-v0.20.0`) must bump past this to expose it; until then an unknown `ValueMatcherID` never matches, so the failure mode is an empty panel, not a crash. |
| **Stale-grant repair wired to the dispatch gate** — three "unused" build warnings in `crates/host/src/authz/` turned out to be the only visible trace of a shipped-but-UNREACHABLE security fix. | auth-caps | post-S10 | **shipped** (2026-07-28) | [builtin-role-freshness](scope/auth-caps/builtin-role-freshness-scope.md) (sibling problem) | [stale-grant-repair](sessions/auth-caps/stale-grant-repair-session.md) | `refresh_grants_if_denied` + `Principal::with_live_grants` were written, fully documented, and **never connected to a gate**; `USER_PREFIX` read as dead only because its sole user was that dead function, so wiring it cleared all three warnings at once. **The bug it closes:** a session token is a CACHED projection of `resolve_caps` taken at login, so a grant written afterwards is invisible until it expires — and `grant_ui_scope_to_admin` runs on EVERY extension install. Live symptom recorded in the code: `modbus` 0.1.7→0.1.8 added `device.status`/`point.status`, the install granted both, and an operator session minted minutes earlier was refused both — every device on the page rendering `unknown` for up to the 12h token lifetime, with nothing in any log. **Distinct from this scope's own fix:** that one keeps the stored ROLE ROW fresh (wired, shipped); this one keeps the TOKEN fresh. One question apart, so the scope now carries a "Sibling problem" section. **The fix:** one insertion in `dispatch_at_depth` before the gates, plus a `gate_for` helper — placed there because there are TWO gates (host-native, and `lb_mcp`'s own `authorize` for `<ext>.<tool>`) and the observed bug is on the EXTENSION side, so wiring only the host-native one would have looked done while leaving the report unfixed; `dispatch_at_depth` is the one point both tiers cross and the one holding the `&Store` `lb_mcp` correctly lacks. `principal` is SHADOWED, not merely gated on: host verbs re-check their own cap internally, so refreshing only the outer gate would pass it and then deny inside. **Safety:** cached caps consulted first (an allowed call never touches the store), bounded by exactly what a re-login would mint, refuses delegated/run-scoped principals, returns `Some` ONLY when the verdict flips, never narrows (revocation rides the `token_revoke` tombstone). **Cost, stated:** a denied `user:` call now does an O(teams) store read — bounded and already an error path, but an authenticated caller can spam denials to force reads. **Tests:** new `stale_grant_repair_test` 4 (repairs · does not widen even when a grant row for that CAP exists for another subject · never re-widens a delegated principal · the repaired identity reaches the verb's OWN inner gate). Revert-checked honestly: 2 of the 4 go red without the wiring; the other 2 assert DENIAL, which is also the un-wired behaviour, so they are future-widening guards rather than proof — stated rather than glossed. `cargo build --workspace` now emits **zero** warnings (was 3); fmt + clippy clean in every touched file; lb-host lib 376 + versions/authz/undo/catalog suites green. |
| **Entity version history + restore (#112)** — the newest N (default 20, admin-adjustable) full after-image snapshots of every dashboard, flow, and rule, with `versions.list`/`.get`/`.restore` + `.config.get`/`.set`, and the structural `Secret<T>` never-in-a-snapshot guard it was gated on. | versions | S10 follow-on | **tested** (2026-07-28; working tree, not git-committed, **unreleased**) | [entity-version-history](scope/versions/entity-version-history-scope.md) | [entity-version-history](sessions/versions/entity-version-history-session.md) | **Generic by construction:** a kind is a data ROW in `crates/host/src/versions/plan.rs` (`kind → table, save_tool, id keys, counter field, hash-ignore`); nothing below that file matches on a kind name — the gateway treats `kind` as an opaque path segment, the descriptors derive their `enum` from the table, and `undo_capture` reaches it through `table_for_kind`. **Capture** is a sibling of `undo_capture` at the same depth-0 chokepoint, sequenced after it: undo journals the BEFORE-image, history keeps the AFTER-image, because "restore version 7" means "what it looked like once that save landed". One `lb_store::capped_insert` per capture into `entity_version`, `cap_key = "{kind}:{id}"`, insert+trim in ONE transaction under the per-key lock. **Restore is a forward action** — it re-dispatches the kind's own `*.save` at depth+1, so it inherits every validator, cap, ownership check, audit hook and cache invalidation for free (never a raw store write); it works after delete (the ring outlives the entity — there is no capture on delete) and it appends a NEW HEAD, so history stays append-only and re-restorable. LWW by design, unlike undo's refuse-on-drift. **The prerequisite shipped first:** `crates/store/src/snapshot_guard.rs` — the secret plane is never snapshotted whatever a plan table says, and a secret-shaped leaf REFUSES the snapshot rather than redacting it (a redacted snapshot looks restorable and would write `***` over a live credential). **Caps:** `.list`/`.get`/`.config.get` viewer · `.restore` author + a no-escalation pre-check on the kind's save cap · `.config.set` admin-only. **Two implementation defects, both found by a test, not by reading:** the dedupe never fired (a dashboard stamps `updated_ts` and a flow bumps `version` on every save, so a no-op re-save was byte-different and burned a ring slot → `hash_ignore` on the plan table), and the named deny test PASSED against reverted code (the nested save denies too, so it was asserting a refusal that happens either way → strengthened to assert the property only the pre-check has: a real and a fictional version id must produce the IDENTICAL error, or the deny path is an existence oracle). **Tests** (real infra, no mocks; no test hand-writes a ring row): host lib 20 · store guard 7 · `versions_test` 9 (25→20 ring, concurrency, dedupe, restore roundtrip per kind, validators re-run, restore-after-delete, new head, per-kind cap, catalog) · `versions_authz_test` 8 (the named deny, opaque denial, ws-isolation, role-bundle exposure, the shell's catalog-probe degradation contract, undo-after-restore) — all four load-bearing ones revert-checked. `fmt` + `build --workspace` clean. **Live-verified in the product, not only in the suite:** the downstream shell drives all five verbs through a real browser against a node running this code (4/4 walks green on a cold scratch rig — dashboard restore, flow counter column, restore-after-delete, and a viewer denied with negative controls run both ways). Two defects only the live route could find: a `400` from the axum extractor on an EMPTY restore body — a valid call, rejected before the cap gate, inverting the deny-before-validate ordering `tool_call.rs` is careful about (fixed: raw `Bytes`, parsed in the handler, 3 unit tests); and the fact that restoring ANOTHER author's dashboard needs `dashboard.save_any` (gate-3 ownership sits under the cap the no-escalation check re-demands) — correct and non-silent, but the clean fix is a per-entity `can_restore` on `versions.list`, deferred. **Downstream:** `NubeIO/rubix-ai#55` builds the UI against this via a local `[patch]`. **Follow-up: tag `node-v*` + bump the rubix-ai pin + drop the patch** — nothing is committed or tagged by this session. |
| **CI restored — master had all four jobs red, so PR checks carried no signal.** | build / CI | — | **shipped** (2026-07-26) | — | [ci-red-baseline](sessions/build/ci-red-baseline-session.md) | Four causes, three of them stale config. `build-and-test` was **not** a runner flake: `ld … signal 7 [Bus error]` was the symptom of `No space left on device` while linking **324 integration test binaries** (152 in `lb-host`), each statically pulling in polars + datafusion + surrealdb + typst. Freeing disk was **not sufficient** — measured on the runner, 88 GB free before cleanup and 109 GB after, and it still overran (the earlier "~14 GB" figure came from an outdated spec and was never measured). Fixed by **sharding** the job into 3 matrix legs (`host`/`gateway`/`rest`, `fail-fast: false`) so no one `target/` holds all 324, plus the free-disk step, `debug=0`, `strip=symbols`, `CARGO_INCREMENTAL=0`; `cargo fmt` split into its own seconds-long job. The `ui` job and `deploy/common/Dockerfile`'s `spa-builder` stage both still referenced **`ui/`, deleted in `678503f`** — the `ui` job died at *setup-node*, meaning `packages/*` + `app/sdk` had **zero** frontend CI; it is now a `packages` job on the root lockfile (pnpm **11**), and the image is explicitly **headless** (a product host fills `/usr/share/lazybones/web`). `file-layout` is now a **ratchet**: the 114-file backlog (not ~20 — that count came from a truncated log) is frozen in `rust/scripts/file-size-baseline.txt` and the check fails only on a NEW violation or a baseline file that GREW. Backlog itself is unpaid — see the session doc's follow-ups. **What the restored gate then found: 36 failing tests across 12 files** (host 31, rest 3, gateway 2), all pre-existing and invisible for weeks — `cargo test`'s first-failing-binary abort meant the shards reported only 1+1+1 until `--no-fail-fast` was added. One (`apikey_routes_test`, 6 tests) is fixed here as pure fixture drift; **triaging the other 30 is a separate project** spanning proof-panel, agent personas, federation, store compaction and the CLI — itemised in the session doc. Debug entry: [ci-red-baseline](debugging/build/ci-red-baseline-disk-exhaustion-and-deleted-ui.md). |
| **Managed dashboards — an extension owns a board, humans read it, admins can fix it** — a derived `managedBy` marker, the admin-override triad, and a typed (non-leaking) managed denial. | extensions | post-S10 | **tested** (2026-07-25; working tree, not git-committed) | [ext-managed-dashboards](scope/extensions/ext-managed-dashboards-scope.md) | [ext-managed-dashboards](sessions/extensions/ext-managed-dashboards-session.md) | **Shipped (host half, Ask 2):** `Dashboard.managed_by` / `DashboardSummary.managed_by` (`managedBy`, `#[serde(default, deserialize_with = "null_default")]`) holding the **bare** ext id — additive, defaulted, no migration, a pre-field record round-trips. **Derived from the saving principal, never caller input**, in ONE helper `dashboard/managed.rs::managed_by_of` (strips `ext:` off `owner_sub()`), so a future identity scheme changes one function; a human cannot mark a board, an admin's override cannot blank or steal a marker (rule 10: the host never branches on WHICH ext). **The admin-override triad completed** — `dashboard.save_any` + `dashboard.share_any` beside the shipped `delete_any`: same catalog row shape, same `ADMIN_ONLY_CAPS` tier, same check POSITION (owner first, override attempted only on failure via `&&`), each its own cap, never an ambient admin-role test, and never a bypass of the verb's base cap. **Typed denial** `DashboardError::ManagedDenied(<ext id>)` → `ToolError::DeniedBecause { code: "managed", subject }` → `403` with `denied: managed=<id>`, produced ONLY for a caller who could already READ the board (gate 3 re-checked) — everyone else keeps the opaque `Denied`, so it is not an existence oracle; every other consumer of the new `ToolError` variant collapses it back to `Denied`. **Not done, deliberately:** no host interpretation of the marker, no uninstall cascade-delete (D5), no pack coupling, no merge of human edits. 11 headless tests (`dashboard_managed_test.rs`, `dashboard_override_test.rs`) green on a real store. Ask 1 (`ui-v0.15.0`) was already cut. Shell/consumer halves live in `rubix-ai` + `rubix-ai-extensions`. |
| **Routed MCP dispatch to a NAMED node (#81)** — a caller may name a target node on an MCP call, and an untargeted call to a **multiply-hosted** extension is REFUSED instead of silently coin-flipped. | mcp | post-S10 | **tested** (2026-07-20; working tree, not git-committed) | [routed-node-dispatch](scope/mcp/routed-node-dispatch-scope.md) | [routed-node-dispatch](sessions/mcp/routed-node-dispatch-session.md) | **The hazard, measured before the fix:** two real nodes hosting one ext, 40 identical untargeted calls → a **25/15 split**, no error — `lb_bus::query` took the first reply and the caller could not tell which physical box ran its tool. **Shipped:** `NodeId` (`crates/bus/src/node_id.rs`) — key-expression-safe *by construction* (rejects `/ * $ ? #`, so a `gw-*` id that would silently address the whole fleet cannot exist); minted here but placed as **fleet-presence's** primitive (below `lb-mcp`/`lb-host`) so the two scopes cannot fork node identity. `Target::Remote { node, tools }` + registry `ext → Vec<Target>` — the change that makes a multiply-hosted ext *representable*, hence checkable (before, a second host silently OVERWROTE the first). The guard at **`resolve` on the calling side** (the only place the candidate set exists — a serve-side guard would need cross-node coordination and would silently never fire) → structured `ToolError::Ambiguous { ext, candidates }`. Target rides the **bus key** `mcp/{ext}/{node}/call`, declared **per workspace served**, so "exactly one responder" is true *by construction* not by comment; the shared key is demoted to a mixed-version path. `lb_bus::query` now **enforces** the single-responder invariant (`BusError::MultipleResponders`). `lb_mcp::call_on_node(…)` — **no new capability grammar** (`mcp:<ext>.<tool>:call` unchanged; addressing ≠ authorization) and `authorize` still strictly precedes `resolve`, so a capless caller gets `Denied` and can never enumerate a fleet. New errors → HTTP 409/503/502 across 8 exhaustive match sites. **Severity CORRECTED:** the entry gate found **neither** `serve_ext` **nor** `register_remote_extension` has any production caller (0 vs ~12/3 test callers), so the hazard is **latent, not active** — the urgency is *ordering* (arm the guard before discovery loads the gun), not firefighting; the scope's original "active hazard" framing was wrong and is amended. **Tests:** `routed_ambiguity_test.rs` **12/12 on every completed run**, but the suite still **hangs ~1 run in 6** — NOT root-caused (see the debugging entry; it is Zenoh peer boot cost, ~10.6s per `Node::boot()` even for a test that makes no bus call, and the untouched `cross_node_routing_test` shows the same) — do not read this row as fully green on two real in-process Zenoh peers over loopback TCP (no mocks) — headline regression, targeted-lands-on-named-node (10 rounds each), unknown-target-never-falls-back, **mandatory** caps-deny in its sharp form (deny is byte-identical whether or not the named node exists, so it cannot enumerate a fleet), authorize-precedes-resolve, **mandatory** ws-isolation in its STRONG form (unreachable, not merely refused — only the per-workspace wall supports that), **mandatory** hot-reload, **mandatory** offline (a LIVE node that DROPS, i.e. real queryable retraction, not an unknown id), unchanged fast path, self-targeting, local-wins, shrinking-fleet-recovers. **Every gate revert-checked** by planting the defect it catches: auto-pick → 2 red; `KEY_UNSAFE` emptied → 3 red; Q6 reverted to `ws/*` → **only** the isolation test red (proving the strong assertion genuinely depends on per-workspace declaration and would have been a FALSE claim under the wildcard); authorize/resolve swapped → 2 red; reload stacks-not-swaps → 2 red. **All 8 scope open questions RESOLVED in-doc with reasoning.** Found + fixed a 1-in-5 parallel flake that was the new `MultipleResponders` check firing correctly on a genuine duplicate-node-id collision the harness manufactured — production code was right, and without that check the suite would have passed silently by keeping the first reply ([debugging](debugging/mcp/duplicate-node-ids-across-concurrent-tests.md)). Workspace sweep: every failure accounted for — 4 suites were **missing binaries** wiped with `target/` (`echo-sidecar`/`fleet-monitor`/`federation`; green once built), 2 are the documented pre-existing federation e2e **stack overflows**, and the 8 `agent_persona_*` failures were **proven pre-existing on a clean-HEAD worktree** (identical counts, identical 8 test names). **NOT load-bearing in production yet** → #82 (fleet-presence must ship the ext-hosting announce that gives the guard a real candidate set, + the `targeted_dispatch` flag `NodeTooOld` needs — it is defined but never returned, so an old node currently lies as `NodeUnreachable`); #83 (`agent/invoke` has the same coin flip, un-narrowed by even an ext segment). No tag cut. **Unblocks ems gateways slice 2** via `lb_mcp::call_on_node(&registry, &bus, &principal, ws, "<ext>.<tool>", input_json, &node_id)`. |
| **Email/password login + workspace selection (the Slack front door) — backend (Phase 1)** — a globally-unique case-insensitive `email` + one global argon2id password on the `_lb_identity` identity; admin verbs `identity.set_email`/`identity.set_password` (gated `mcp:identity.manage:call`) + self-service `POST /auth/password`; the three gateway routes `POST /auth/login {email,password}` (verify globally → enumerate effective memberships → 0→403 / 1→full token / N→select-token + roster), `POST /auth/select {workspace}`, `POST /auth/switch {workspace}`. | auth-caps | post-S10 (core-auth-caps) | **tested** (2026-07-16) · **removal sweep DONE 2026-08-03** — `/auth/*` is now the ONLY human door | [email-login](scope/auth-caps/email-login-scope.md) | [email-login](sessions/auth-caps/email-login-session.md) | **`lb-authz`:** `Identity.email` (lower-cased) + `fold_email` + a **race-safe unique index** `identity_email:{folded}` via `store::create` (Conflict-on-duplicate, not read-then-write); `identity_by_email` case-insensitive lookup; new **`identity_credential.rs`** global credential record in `_lb_identity`. **`lb-host`:** new **`identity_credential/`** service (`set` admin verb · `change` self-service · `verify` = `global_credential_verify`, **timing-uniform** — argon2 burned against a dummy hash on unknown/absent so email-enumeration has no latency oracle · MCP bridge); `identity/` gains `email` in the view, `set_email`, `by_email`, and **`login_workspaces`** (un-gated enumeration: effective member AND not disabled, `{ws,name}`). **`lb-role-gateway`:** `GlobalCredentialCheck` trait (`GlobalPasswordHash`/`GlobalDevTrustAny`, selected by `LB_DEV_LOGIN` like the per-ws seam); the **select-token** (`ws:""`,`caps:[]`,`constraint:["ws-select"]`,~5-min TTL — powerless everywhere, positively accepted ONLY at `/auth/select`); **`mint_full_session`** = the one role-correct issuance path factored out of `login.rs` (viewer floor ∪ `resolve_caps_live` ∪ nav-reach) so `/login` + all `/auth/*` mint byte-identically; `/auth/login|select|switch|password` + `auth_reply.rs`; per-email failure rate limit (10/15min, `FixedWindowLimiter::peek`); admin REST `/admin/identities` (accepts `email`) + `/{sub}/email` + `/{sub}/password`. **Decisions:** all 3 open questions resolved (dev-login honored · 10/15min per-email · `{ws,name}` roster). **Tests (real gateway+store, argon2 real, no mocks):** `email_login_test` 6 (1/0/N branch · **unknown-email==wrong-password 401 body** · email uniqueness+case-insensitive · self-service rotate) + `email_login_deny_test` 3 (select-token refused everywhere but `/auth/select` · full token refused at `/auth/select` · switch-to-non-member 403) + unit `select_token` 3 + `rate_limit::peek` 1. **Regression green:** `login_hardening`/`identity_routes`/`gateway`/`admin_routes`/`nav_reach`/`viewer_reach` + gateway `--lib` + `lb-authz`/`lb-host --lib` + `cargo build --workspace`. **Removal sweep — DONE 2026-08-03:** `POST /login` **deleted** (it checked membership BEFORE the credential, an unauthenticated account-enumeration oracle) along with the gateway's per-ws `CredentialCheck`/`DevTrustAny`/`PasswordHash` seam, `membership_login_resolve` (bootstrap-on-empty = a self-promotion hazard once legacy rows stopped counting), and the whole legacy `user` record family (`host/src/users/`, `/admin/users*`, `mcp:user.manage`/`user.disable`). `lb login` re-pointed to `/auth/login` (+`/auth/select` on N>1, `--email`/`--password`|`LB_LOGIN_PASSWORD`); ~12 suites ported to `mint_full_session`/explicit provisioning or retired. **Lost:** per-workspace disable/enable (no membership-row equivalent; `membership.remove` is stronger). **Still standing:** the per-ws `credential` RECORD + `identity.set_credential` — no longer a login door, kept only as the invite flow's takeover-protection state. **Deferred:** email-verify/reset/MFA/OIDC unchanged deferrals; rubix-ai `[patch]` wire (Phase 2) + UI (Phase 3) are downstream. |
...
| admin-crud destructive + user lifecycle *(the `user.*` half REMOVED 2026-08-03)* | auth-caps | S9+ | **shipped** (workspace/team/member) · **removed** (user CRUD) | [admin-crud](scope/auth-caps/admin-crud-scope.md) | [admin-crud](sessions/auth-caps/admin-crud-session.md) | **slice 2** — the destructive half + a real dev-store **user CRUD**. New host **`users`** svc: `UserRecord{user,active,role,cred_ref}` per `(ws,user)` + credential-free `UserView` (`cred_ref` never serialized); `user.create`/`list`/`disable`/`enable`/`delete` (gated `mcp:user.manage`/`user.disable`); **`user_login_check`** = the un-gated pre-mint seam wired into `POST /login` so **disable bites minting** (absent record still auto-seeds). Workspace lifecycle: `rename`(+un-archive)/`delete`(soft archive, hidden from list)/**`purge`** (hard: distinct `mcp:workspace.purge` cap **AND** typed confirm token; directory tombstone, no resurrection). `teams.delete` (cascade: drop member edges + `revoke_subject` + tombstone, returns count) + `teams.rename`; `members.remove`. `user.delete`/`teams.delete` call slice-1's **`revoke_subject`** seam (one revoke path). Gateway: `/admin/*` routes + `DELETE /teams/{team}/members/{user}`, each **re-checks the cap server-side** (UI gate is convenience); dev claim set now admin. `http.ts` gains every verb (+ `delJson`) + `admin.fake.ts` 1:1. +7 host (deny-per-verb · 2-ws iso · soft-before-hard+confirm · disable-bites-login · delete-revokes-grants · teams-cascade · tombstone-not-resurrected) +3 gateway (**server-deny-on-forged-call** · admin round-trip · login-refuses-disabled) ; 40 Vitest + tsc green; cargo build/fmt/file-size green; no SDK/WIT change. **2026-08-03 — the `user.*` half is DELETED** (pre-production legacy sweep, [email-login](scope/auth-caps/email-login-scope.md) §Sequencing): `host/src/users/` (the whole svc + `user_login_check`), `/admin/users*`, `mcp:user.manage:call`/`mcp:user.disable:call`, **and `POST /login`** with its per-ws `CredentialCheck` seam and `membership_login_resolve` bootstrap-on-empty. Cause: the legacy `user` row was a SECOND source of truth for membership, keyed **bare** on write but read **prefixed** by the login path, so `GET /admin/members` listed a person `/auth/login` then refused ([debugging](debugging/app/roster-login-disagree-legacy-user-rows.md)); `/login` was also an unauthenticated **account-enumeration oracle** (membership checked before the credential). Roster = `membership.*` only; `/auth/*` is the only human door; first admin provisioned explicitly. **Lost: per-workspace disable/enable** (no membership-row equivalent; `membership.remove` is the stronger surviving control). New invariant test: `identity_membership_test::roster_and_login_path_agree_on_the_one_membership_source`. Workspace/team/member verbs above are unaffected |
