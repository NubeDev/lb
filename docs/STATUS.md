...
**extensions/host-callback** (scoped 2026-06-27, NOT built — the **forever-ABI** addition: a WASM
**guest** gains one WIT import `host.call-tool(name,input)` so it can call host MCP tools
(inbox/outbox/db/series/other extensions) under its delegated `caller ∩ grant` authority — the
symmetric backend dual of the page's `POST /mcp/call` bridge. Today a guest is a one-way box (the WIT
imports only `host.log`), so an extension backend that ingests/reads/reacts can't be written as a guest
— it has to live in a host service. Reuses S5 `Principal::derive` + the `call_tool` chokepoint; carries
identity into `HostState` per-call. WIT minor bump `@0.2.0`, world major unchanged so `0.1.0` guests
keep loading; flagged §11.2 forever-commitment).
