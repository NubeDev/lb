...
|---|---|---|---|---|---|---|
| **A curated menu ROW is pinnable — `nav:<navid>/<rowid>`** — a board bound to variables ("Chullora › Energy") and a folder that opens its own board had no pin ref, so a site menu drew no pin anywhere. | nav | post-S10 | **tested** (2026-09-17, branch `feat/nav-row-pins`, unreleased — needs the next `node-v*` tag) | [nav-row-pins](scope/nav/nav-row-pins-scope.md) | [nav-row-pins](sessions/nav/nav-row-pins-session.md) | `NavItem.id` (kept / minted / duplicate re-minted / malformed `BadInput` in `nav.save`, `row_ids.rs`); `ResolvedItem` gains `id` (echoed by `resolve_item`), and on a row pin `nav_id` + `trail`; a fifth pin grammar resolved in `resolve_row_pin.rs` through the pick tiers' readability gate + the ordinary `resolve_item` pipeline (a folder pins as its board; hide beats pin; every miss strips silently). No new caps/verbs; no migration — an old nav's rows gain ids on their next save. Tests: `nav_row_pins_test` 4 new; `nav_test` 50 (one round-trip assertion now clears minted ids), `reusable_pages` 8, `nav_home` 8, `nav_footer` 3, `nav_context_builtins` 6, `nav_ext_boards_gate` 4, lib `nav` 35, gateway `nav_default_route` 3 + `nav_reach` 2 — all green. |
