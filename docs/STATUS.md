...
|---|---|---|---|---|---|---|
| **Widget catalog + save-validation (widget-platform Slice A)** — backend-owned `widget_catalog.json` served over a new `dashboard.catalog` MCP verb (built-in view palette + per-view config schema + ext tiles + genui components) + host-side `dashboard.save` rejection of unknown `view` kinds (closes G4: the AI hallucinating views the save then accepts). | widgets / frontend | S9+ | **building** | [slice scope](scope/frontend/dashboard/widget-catalog-scope.md) · [umbrella](scope/widgets/widget-platform-scope.md) | — | Catalog JSON hand-authored in `host/src/dashboard/`; verb (`catalog.rs`) + validator (`views.rs`) + `mcp:dashboard.catalog:call` member-cap wiring still to build. Enforcement is view-NAME only this slice (option-schema validation is a named follow-up). |
