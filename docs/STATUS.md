...
|---|---|---|---|---|---|---|
| **CI restored — master had all four jobs red, so PR checks carried no signal.** | build / CI | — | **shipped** (2026-07-26) | — | [ci-red-baseline](sessions/build/ci-red-baseline-session.md) | Four causes, three of them stale config. `build-and-test` was **not** a runner flake: `ld … signal 7 [Bus error]` was the symptom of `No space left on device` while linking ~200 debug test binaries (polars + surrealdb + rusqlite each) — fixed with a free-disk step (~25 GB) + `CARGO_PROFILE_{DEV,TEST}_DEBUG=0` + `CARGO_INCREMENTAL=0`. The `ui` job and `deploy/common/Dockerfile`'s `spa-builder` stage both still referenced **`ui/`, deleted in `678503f`** — the `ui` job died at *setup-node*, meaning `packages/*` + `app/sdk` had **zero** frontend CI; it is now a `packages` job on the root lockfile (pnpm **11**), and the image is explicitly **headless** (a product host fills `/usr/share/lazybones/web`). `file-layout` is now a **ratchet**: the 114-file backlog (not ~20 — that count came from a truncated log) is frozen in `rust/scripts/file-size-baseline.txt` and the check fails only on a NEW violation or a baseline file that GREW. Backlog itself is unpaid — see the session doc's follow-ups. Debug entry: [ci-red-baseline](debugging/build/ci-red-baseline-disk-exhaustion-and-deleted-ui.md). |
