...
**Newly scoped (2026-07-04): the React Native mobile app (`docs/scope/app/`, workshop `app/`).**
Three asks, not yet built: `app-shell-scope.md` (RN host — many workspaces, **REST + SSE via the
gateway**, zenoh-ts rejected as an unmediated second surface), `app-extensions-scope.md` (Re.Pack 5 +
Module Federation 2 remotes via an additive `[app]` manifest block, JS-only, signed-`Artifact` publish;
reference exts `proof-panel-app` + `channel-chat`), `app-sdk-scope.md` (`@nube/app-sdk` as the authored
contract source; aligns with the `panel-kit` promotion above toward one shared panel/widget SDK).
Session: `sessions/app/app-scope-setup-session.md`. First implementation slice: the shell.
