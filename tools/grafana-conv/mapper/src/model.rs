//! **VENDORED MIRROR** of `rust/crates/host/src/dashboard/model.rs`.
//!
//! This file is a byte-for-byte copy of the host's `Dashboard`/`Cell`/`Variable`
//! record types, kept in this standalone workspace so the converter emits *exactly*
//! the shape the host persists — without dragging the host crate (and its deps)
//! into a tool that owns no token / store / cap. The fold-in (Stage 2,
//! `viz/import-export-scope.md`) deletes this file and path-depends on the host
//! crate; until then `tests/mirror_sync.rs` guards that this stays in sync.
//!
//! NOTE: do NOT `cargo fmt` this file — it must stay byte-synced with the host's
//! copy. The `@generated` marker below makes rustfmt skip it; `cargo fmt --all`
//! is safe to run on the rest of the workspace.
//!
//! @generated
//! ─────────────────────────────────────────────────────────────────────────────
//!
