 ...
 *  verbs through its own client; `weather` is self-sourced from the fixed `weather.current` verb, with
 *  the location entered on the Options step), so the wizard's Source step is optional for them and the
 *  "pick a source" gate must not block advancing. A source-bound view (every viz, genui, template) is
 *  absent. */
export const SOURCELESS_VIEWS: ReadonlySet<string> = new Set(["insights", "weather"]);
